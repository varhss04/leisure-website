...
const BlogNeonBooks = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/blog-neon-books')
  ...
  const savePost = () => {
    if (newPostContent.trim()) {
      axios
        .post('http://localhost:5000/blog-neon-books', { content: newPostContent })
        .then((response) => {
  ...
  const deletePost = (id) => {
    axios
      .delete(`http://localhost:5000/blog-neon-books/${id}`)
