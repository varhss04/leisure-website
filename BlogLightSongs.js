...
const BlogLightSongs = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/blog-dark-songs')
  ...
  const savePost = () => {
    if (newPostContent.trim()) {
      axios
        .post('http://localhost:5000/blog-dark-songs', { content: newPostContent })
        .then((response) => {
  ...
  const deletePost = (id) => {
    axios
      .delete(`http://localhost:5000/blog-dark-songs/${id}`)
