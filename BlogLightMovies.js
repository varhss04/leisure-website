...
const BlogLightMovies = () => {
  ...
  useEffect(() => {
    ...
    if (vibe === 'movies' && aesthetic === 'light') {
      setValidPage(true);
    } else {
      setValidPage(false);
    }
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/blog-dark-movies')
  ...
  const savePost = () => {
    if (newPostContent.trim()) {
      axios
        .post('http://localhost:5000/blog-dark-movies', { content: newPostContent })
        .then((response) => {
  ...
  const deletePost = (id) => {
    axios
      .delete(`http://localhost:5000/blog-dark-movies/${id}`)
  ...
  return (
    <div style={styles.body}>
      <div style={styles.header}>
        <div style={styles.brand}>Leisure movies</div>
...
const styles = {
  body: {
    backgroundImage: 'url("lightblog.jpg")',
...
export default BlogLightMovies;
