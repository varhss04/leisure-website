...
const BlogNeonMovies = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/blog-neon-movies')
  ...
  const savePost = () => {
    if (newPostContent.trim()) {
      axios
        .post('http://localhost:5000/blog-neon-movies', { content: newPostContent })
        .then((response) => {
  ...
  const deletePost = (id) => {
    axios
      .delete(`http://localhost:5000/blog-neon-movies/${id}`)
